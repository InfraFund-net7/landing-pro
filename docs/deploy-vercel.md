...
- **`/admin` ↔ `/admin/login` infinite loop after successful login:** Caused by `/admin` always redirecting to login when users exist (fixed in latest `develop` — `/admin` only redirects to create-first-user when the DB has no users). Redeploy and clear cookies for the site.
