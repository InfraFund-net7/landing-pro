        ...
        <div>
          ...
          transition-all duration-500 ease-out`}
            >
                {children}
                <ChevronDownIcon
                    className={`pointer-events-none size-4 shrink-0 translate-y-0.5 transition-transform duration-500 ease-out ${open ? "rotate-180" : ""
                        }`}
                />
            </button>
...
export function AccordionContent({ children, open }: AccordionContentProps) {
    return (
        <div
            className={`overflow-hidden text-sm transition-all duration-500 ease-in-out ${open ? "max-h-96 opacity-100" : "max-h-0 opacity-0"
                }`}
        >
