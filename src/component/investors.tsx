...
export default function Investors() {
    ...
    return (
        <div className="flex flex-col justify-center items-center gap-12">
            <FadeInStagger className="w-full h-[900px] relative flex justify-start items-center overflow-hidden 
                    ...
                    <div className="max-w-[820px] flex flex-col justify-center items-start gap-12 text-white 
                      max-md:items-center max-md:text-center">
                        <h1 className="text-[64px] font-bold leading-tight max-lg:text-[48px] max-md:text-[32px]">
                            Invest Directly in the <br className="max-md:hidden" /> Future of Our Planet
                        </h1>
                      ...
                      max-lg:w-full max-lg:border-r-0 max-lg:border-b max-lg:py-12`}
                            >
                                <div className="flex flex-col gap-12 justify-center items-center max-md:gap-6">
                                    <Image src={item.icon} alt={item.title} width={100} height={100} className="max-md:w-[70px]" />
                                    <h2 className="text-4xl font-bold max-md:text-2xl">{item.title}</h2>
                                </div>
                                <p className="text-2xl font-normal max-md:text-base">{item.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
                ...
                <div className="flex flex-col justify-center items-center gap-10 p-10 w-full relative max-md:p-6">
                    ...
                    <div className="flex flex-col gap-8 justify-center items-center">
                        {invest.map((item, index) => {
                            ...
                            return (
                                <div key={index} className="flex flex-col items-center gap-6 max-md:gap-4">
                                    <div className="flex flex-col items-center gap-4 text-center max-w-md">
                                        <div className="flex justify-center items-center gap-4 text-white">
                                            <Icon className="w-11 h-11 max-md:w-8 max-md:h-8" />
                                            <h3 className="text-[32px] font-medium max-md:text-xl">{item.title}</h3>
                                        </div>
                                        <p className="text-white/90 text-2xl font-normal max-md:text-base">{item.description}</p>
                                    </div>
