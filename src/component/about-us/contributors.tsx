...
const ContributorCard = ({ item, linkedin }: { item: Contributor; linkedin: string }) => {
  ...
  "
        ...
        <div className="text-center w-full min-h-[60px] flex flex-col justify-start">
          ...
          <h4 className="text-xs text-muted-foreground text-white text-center mt-1 line-clamp-2">{item.role}</h4>
        </div>
      </div>
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} width="90vw" height="auto" className="sm:hidden">
        <div className="flex flex-col items-center gap-4 mt-4">
          ...
          <h3 className="text-lg font-semibold text-center text-white">{item.name}</h3>
          <p className="text-sm text-gray-300 text-center">{item.role}</p>
          <p className="text-sm text-center leading-relaxed text-white">{item.description}</p>
