    ...
    const toggleFaq = (id: string) => {
        setOpenState((prev: OpenState) => {
            if (allowMultiple || prev.type === 'multiple') {
                const newIds = new Set<string>(prev.type === 'multiple' ? prev.ids : new Set<string>())
                if (newIds.has(id)) {
                    newIds.delete(id)
                } else {
                    newIds.add(id)
                ...
                return (
                    ...
                    >
                        <div className="w-full flex items-center justify-between px-6 py-6 text-left transition-colors hover:text-slate-200 max-md:px-4 max-md:py-4">
                            <span className="text-lg font-bold max-md:text-base">{faq.question}</span>
                            <svg
                                className={`w-5 h-5 transition-transform duration-500 ease-out ${isOpen(faq.id) ? 'rotate-180' : 'rotate-0'
                                    }`}
                                fill="none"
                                viewBox="0 0 24 24"
                                stroke="currentColor"
