...
function getAxiosRequestConfig(
  ...
  const captcha =
    extraHeaders['X-Captcha-Token'] ?? extraHeaders['x-captcha-token'];
  if (captcha !== undefined && (!captcha || !captcha.trim())) {
    throw new Error('X-Captcha-Token header is empty');
  }
  return {
    headers: {
...
const apiService: ApiService = {
  ...
  post: async <T>(
    ...
    try {
      ...
      const response = await _axios.post<T>(
        reWriteUrl(url),
        data ?? {},
        config
      );
